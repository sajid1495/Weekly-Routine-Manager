...
import 'package:wrtnm/data/database5.dart';
import 'package:wrtnm/util/dialog_box.dart';
import 'package:wrtnm/util/toto_tile.dart';

class DayTaskPage5 extends StatefulWidget {
  final String day;

  DayTaskPage5({required this.day});

  @override
  _DayTaskPageState createState() => _DayTaskPageState();
}

class _DayTaskPageState extends State<DayTaskPage5> {
  ...
  void initState() {
    ...
    if (_myBox.get("TODOLIST5") == null) {
      db5.createInitialData();
    } else {
      db5.loadData();
    }
  ...
  void saveNewTask() {
    setState(() {
      if (_controller.text != "") {
        db5.toDoList5.add(_controller.text);
        _controller.clear();
      }
    });
